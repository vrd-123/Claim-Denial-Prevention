...
# MAGIC %pip install "numpy<2" "scipy<1.14" "xgboost==3.2.0" "shap>=0.51.0"
...
# ── Feature set — must match the training notebook's feature order exactly ────
# IMPORTANT: This order must match the order the model was trained with.
# The model expects diag_category_enc in position 8 (after diag_claim_count).
FEATURE_COLS = [
    ...
    "diag_claim_count",         # frequency of this diagnosis code in dataset
...
# MAGIC
# MAGIC SHAP 0.51.0+ natively supports XGBoost 3.x models, including the array-format `base_score`.
# MAGIC No manual patching is required.
...
# Workaround for SHAP 0.48.0 + XGBoost 2.1.4 incompatibility:
# TreeExplainer cannot parse XGBoost 2.x base_score format.
# Using model-agnostic Explainer with the sklearn wrapper as a fallback.
explainer = shap.Explainer(xgb_model.predict_proba, X)
...
# For binary classification, we want the class-1 (Approved) direction
if hasattr(raw_shap, 'values'):
    if raw_shap.values.ndim == 3:
        # Shape: (n_claims, n_features, n_classes) → take class 1
        shap_values = raw_shap.values[:, :, 1]
        print("  ℹ  SHAP returned 3-D array — slicing class-1.")
    else:
        # Shape: (n_claims, n_features)
        shap_values = raw_shap.values
        print("  ℹ  SHAP returned 2-D array.")
else:
    # Fallback for older API
    shap_values = raw_shap
    print("  ℹ  SHAP returned raw values.")
...
REASON_MAP = {
    ...
    "diag_category_enc": (
        "The diagnosis category classification is inconsistent with the expected clinical billing pattern.",
        "The diagnosis category classification aligns with the expected clinical billing pattern.",
    ),
    "is_billed_missing": (
...
display(
    explanations_pd[[
        ...
        "reason_1_feature", "reason_1_text", "reason_1_impact", "reason_2_feature", "reason_2_text", "reason_2_impact", "reason_3_feature", "reason_3_text", "reason_3_impact", "processed_at"
    ]].head(3)
)
